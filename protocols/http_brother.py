...
# ═════════════════════════════════════════════════════════════════════════════
# Cycle event detection  (preserved verbatim from former events.py)
# ═════════════════════════════════════════════════════════════════════════════
...
def _detect_cycle_events(machine_id, machine_name, pallet_count, ts, data):
    ...
    for pallet in pallets_to_check:
        ...
        prev_running = (prev["op_end"] == "")
        curr_running = (current["op_end"] == "")

        if prev_running is False and curr_running is True:
            program_for_payload = current["program"] or prev.get("last_program") or ""
            events.append({
        ...
        elif (prev_running is True and curr_running is False) or \
             (not prev_running and not curr_running and current["op_end"] != prev["op_end"]):
            program_for_payload = current["program"] or prev.get("last_program") or ""
            events.append({
