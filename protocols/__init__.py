...
Plugins MAY expose:
    ...
    def force_close_cycles(machine_id: str, machine_name: str,
                           ts: str, config: dict) -> list[dict]
        # Called by the poller when a machine has been unreachable long
        # enough that any in-progress cycles should be closed. Returns
        # event dicts (just like detect_events) — typically one or more
        # cycle.completed events that the poller will dispatch.
        # If the plugin doesn't implement it, the poller assumes no
        # closeout events are needed.
...
def dispatch_force_close(machine_id: str, machine_name: str, ts: str,
                         config: dict) -> list[dict]:
    """
    Ask the plugin to emit closeout events for any cycles still believed
    to be in progress for this machine. Called by the poller when a
    machine has been unreachable for the watchdog timeout.

    Plugins that don't implement force_close_cycles return [] silently.
    """
    pid = config.get("protocol", "")
    mod = _registry.get(pid)
    if not mod:
        return []
    fn = getattr(mod, "force_close_cycles", None)
    if fn is None:
        return []
    try:
        return list(fn(machine_id, machine_name, ts, config))
    except Exception as e:
        logger.error(f"[{machine_name}] force_close_cycles in '{pid}' failed: {e}")
        return []
