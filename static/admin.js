...
(function () {
  ...
  function renderTable() {
    ...
    function fmtDriftCell(off) {
      if (off === null || off === undefined) return '0';
      const sign = off > 0 ? '+' : (off < 0 ? '−' : '');
      const abs = Math.abs(off);
      if (abs === 0) return '0';
      if (abs === Math.floor(abs)) return sign + abs + 'h';
      return sign + abs + 'h';
    }
    ...
    tbody.innerHTML = machines.map(m => `
      <tr>
        ...
        <td class="mono muted">${fmtDriftCell(m.utc_offset)}</td>
        <td>
          <div class="actions">
  ...
  // Test connection
  // ══════════════════════════════════════════════════════════════════════
  function fmtDriftHours(hours) {
    // Render a drift value as a friendly hours-and-minutes string.
    if (hours === null || hours === undefined) return '';
    const sign = hours >= 0 ? '+' : '−';
    const abs = Math.abs(hours);
    const wholeHours = Math.floor(abs);
    const minutes = Math.round((abs - wholeHours) * 60);
    if (wholeHours === 0 && minutes === 0) return '0';
    if (minutes === 0) return `${sign}${wholeHours}h`;
    if (wholeHours === 0) return `${sign}${minutes}min`;
  ...
  async function testConnection() {
    ...
    try {
      ...
      if (r.reachable) {
        ...
        // Clock comparison + drift suggestion
        if (r.controller_clock) {
          msg += '\n';
          msg += '\n  ──── Clock Check ────';
          msg += '\n  Time on the machine panel: ' + r.controller_clock;
          if (r.server_clock_local) {
            msg += '\n  Local time right now:      ' + r.server_clock_local;
          }
          // Accept new or legacy field name from the API
          const sugg = (r.suggested_clock_offset !== undefined && r.suggested_clock_offset !== null)
                       ? r.suggested_clock_offset
                       : (r.suggested_utc_offset !== undefined ? r.suggested_utc_offset : null);
          if (sugg !== null) {
            const cur = parseFloat(document.getElementById('mUtcOffset').value);
            const curDisplay = isNaN(cur) ? 0 : cur;
            msg += '\n  Suggested clock drift:     ' + fmtDriftHours(sugg);
            msg += '\n  Currently configured:      ' + fmtDriftHours(curDisplay);
            if (Math.abs(sugg - curDisplay) > 0.01) {
              if (Math.abs(sugg) < 0.01) {
                msg += '\n  ✓ Machine clock matches local time. Click "Apply" below to set drift to 0.';
              } else {
                const absSugg = Math.abs(sugg);
                if (absSugg >= 6) {
                  msg += '\n  ⚠ Machine clock is off by ~' + Math.round(absSugg) + ' hours. Best to fix at the panel.';
                } else if (absSugg < 0.5) {
                  msg += '\n  ⚠ Machine clock is slightly off. Click "Apply" below to compensate, or fix at the panel.';
                } else {
                  msg += '\n  ⚠ Machine clock is off by ~' + fmtDriftHours(sugg).replace(/^[+−]/, '') + '. Click "Apply" below or fix at the panel.';
                }
              }
            } else {
              msg += '\n  ✓ Configured drift matches the machine.';
            }
          }
        }
        ...
        // One-click "Apply Suggested Drift" button if applicable
        const sugg = (r.suggested_clock_offset !== undefined && r.suggested_clock_offset !== null)
                     ? r.suggested_clock_offset
                     : (r.suggested_utc_offset !== undefined ? r.suggested_utc_offset : null);
        if (sugg !== null) {
  ...
  function attachApplyOffsetButton(suggestedOffset) {
    const resultEl = document.getElementById('testResult');
    if (!resultEl) return;
    // Remove any prior button so consecutive Test clicks don't stack them
    const existing = document.getElementById('btn-apply-offset');
    if (existing) existing.remove();

    const btn = document.createElement('button');
    btn.id = 'btn-apply-offset';
    btn.className = 'btn btn-secondary';
    btn.style.marginTop = '8px';
    btn.textContent = `Apply suggested drift (${fmtDriftHours(suggestedOffset)})`;
    btn.addEventListener('click', () => {
      document.getElementById('mUtcOffset').value = String(suggestedOffset);
      btn.remove();
      resultEl.textContent = (resultEl.textContent || '')
        + `\n  ✓ Drift updated to ${fmtDriftHours(suggestedOffset)} — click Save Machine to persist.`;
    });
