...
def _ensure_config():
    ...
    if not AUTH_FILE.exists():
        AUTH_FILE.write_text(json.dumps({
            "username": "admin",
            "password_hash": _hash_pw("HenryShop224")
        }, indent=2))
...
async def api_machines():
    """Returns machines in saved display order."""
    machines = _apply_order(_load_machines())
    return [{"id": m["id"], "name": m["name"], "protocol": m["protocol"], "utc_offset": m.get("utc_offset", 0)} for m in machines]
